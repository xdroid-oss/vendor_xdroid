...
function xdroidremote()
{
    ...
    git remote rm xdroid 2> /dev/null
    ...
    then
        ...
        local PFX="xdroid-oss/"
    else
        local PROJECT=$REMOTE
    fi

    local XDROID_USER=$(git config --get review.xdroidoss.org.username)
    if [ -z "$XDROID_USER" ]
    then
        git remote add xdroid ssh://review.xdroidoss.org:29418/$PFX$PROJECT
    else
        git remote add xdroid ssh://$XDROID_USER@review.xdroidoss.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'xdroid' created"
}
...
function xdroidgerrit() {
    ...
    local user=`git config --get review.xdroidoss.org.username`
...
function xdroidrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "xdroidOSS Gerrit Rebase Usage: "
        echo "      xdroidrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.xdroidoss.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
